...
- **When to use**: bootstrapping `.knowledge` for an existing repository,
  repairing incomplete entity coverage, assimilating `.knowledge/` intake,
...
- Existing knowledge: `.knowledge/entities/*/_entity.yaml`,
  ...
  `.knowledge/ontology/schema.md`.
- Documentation intake roots if present: `.knowledge/` and `.documenation/`.
...
If `.knowledge/` or `.documenation/` exists, classify each document by
content:
