...
export class FooterLinkValidator {
  ...
  async validateAllFooterLinks(): Promise<LinkValidationResult[]> {
    ...
    const internalLinks = [
      ...
      { link: '#texecon/mark-hazleton', pageId: 'texecon/mark-hazleton', label: 'About Mark Hazleton' },
      { link: '#texecon/jared-hazleton', pageId: 'texecon/jared-hazleton', label: 'Dr. Jared Hazleton' },
    ...
    const sectionLinks = [
      { link: 'home', label: 'Home', isSection: true },
    ];
    ...
    for (const item of internalLinks) {
      ...
      results.push({
        ...
        type: 'internal',
        message: pageExists ? `${item.label} page exists` : `${item.label} page not found in navigation data`
      });
    }
    ...
    for (const item of externalLinks) {
      try {
        ...
        results.push({
          ...
          message: response.ok ? `${item.label} is accessible` : `${item.label} returned ${response.status}`
        });
      } catch (error) {
        results.push({
          link: item.link,
    ...
    for (const item of sectionLinks) {
      ...
      results.push({
        ...
        type: 'section',
        message: sectionExists ? `${item.label} section exists` : `${item.label} section not found in DOM`
      });
    }
